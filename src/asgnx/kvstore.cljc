...
(defn state-put [m ks v]
  (cond
    (> (count ks) 1) (assoc m (first ks) (state-put (get m (first ks)) (rest ks) v))
    (= (count ks) 1) (assoc m (first ks) v)
    :else (assoc m nil v)))
...
(defn state-remove [m ks]
  (cond
    (> (count ks) 1) (let [res (state-remove (get m (first ks)) (rest ks))]
                         (if (not (empty? res)) (assoc m (first ks) res)
                           (dissoc m (first ks))))
    (= (count ks) 1) (dissoc m (first ks))
    :else nil))
...
;; See the tests in asgnx.kvstore-test for a complete spec.
;;
(defn state-get ;; Changed signature to take 2 or 3 parameters
  ([m ks] (state-get m ks nil))
  ([m ks res] (get-in m ks res)))
